...
use crate::resources::{
    init_texture_with_data, BufferHandle, ResourceHandle, ResourceHandleConvertable, TextureHandle,
};
use crate::CoGr;
...
impl<'a> Encoder<'a> {
    pub fn to_screen(&mut self, to_screen_texture: &TextureHandle) -> Result<()> {
        let texture = self
            .gpu_context
            .resource_pool
    ...
    pub fn dispatch_pipeline<PushConstants: Pod>(
        ...
        let bind_group_entries = resources
            ...
            .map(|(i, val)| wgpu::BindGroupEntry {
                binding: i as u32,
                resource: match val {
                    ResourceHandle::T(texture_handle) => wgpu::BindingResource::TextureView(
                        self.gpu_context
                            .resource_pool
                            .grab_texture(&texture_handle)
                            .texture_view
                            .as_ref()
                            .unwrap(),
                    ),
                    ResourceHandle::B(buffer_handle) => self
                        .gpu_context
                        .resource_pool
                        .grab_buffer(&buffer_handle)
                        .buffer
                        .as_ref()
                        .unwrap()
                        .as_entire_binding(),
