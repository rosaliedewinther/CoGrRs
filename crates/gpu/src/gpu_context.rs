...
impl GpuContext {
    pub fn new(window: &winit::window::Window, to_screen_texture_name: &str) -> GpuContext {
    ...
    pub fn get_encoder_for_draw(&mut self) -> wgpu::CommandEncoder {
        ...
        if self.to_screen_pipeline.is_none() {
            self.to_screen_pipeline = Some(ToScreenPipeline::new(&self.device, &self.get_raw_texture(&self.to_screen_texture_name)));
        }
        self.device
    ...
    pub fn image_buffer_to_screen(&self, encoder: &mut wgpu::CommandEncoder) {
        ...
        _render_pass.set_index_buffer(self.to_screen_pipeline.as_ref().unwrap().index_buffer.slice(..), wgpu::IndexFormat::Uint16);
        _render_pass.draw_indexed(0..self.to_screen_pipeline.as_ref().unwrap().num_indices, 0, 0..1);
    }
    ...
    pub fn pipeline<const M: usize>(&mut self, shader_name: &str, flags: [&str; M], execution_mode: Execution) {
        ...
        flags.iter().for_each(|flag| cfg.defs.push((flag.to_string(), None)));

        let shader_slice: Vec<u32> = inline_spirv_runtime::runtime_compile(
            &std::fs::read_to_string("./shaders/".to_string() + shader_name + ".comp").unwrap(),
            Some(&("./shaders/".to_string() + shader_name + ".comp")),
            &cfg,
        )
        .map_err(|e| println!("{}", e))
        ...
        let re = Regex::new(r"buffer [^\s\\]*_block|(([ui]*image3D|[ui]*image2D|[ui]*image1D) [a-z_A-Z]*)").unwrap();
        ...
        self.resources.insert(
            ...
            GpuResource::Pipeline(ComputePipeline::new(
                ...
                match execution_mode {
                    Execution::PerPixel1D => (self.size.width * self.size.height / compute_group_sizes.0, 1u32, 1u32),
                    Execution::PerPixel2D => (self.size.width / compute_group_sizes.0, self.size.height / compute_group_sizes.1, 1),
                    Execution::N3D(n) => (n / compute_group_sizes.0, n / compute_group_sizes.1, n / compute_group_sizes.2),
                    Execution::N1D(n) => (n / compute_group_sizes.0, 1, 1),
                },
