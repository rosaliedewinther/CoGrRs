...
impl MainGui {
    pub fn new(device: &wgpu::Device, surface_format: &wgpu::TextureFormat, window: &winit::window::Window, queue: &wgpu::Queue) -> Self {
    ...
    pub fn draw(
            ...
            window
                ...
                .build(|| {
                    ...
                    for (slider_name, sliderdata) in &mut self.slider {
                        Ui::slider(ui, slider_name, sliderdata.min, sliderdata.max, &mut sliderdata.current);
                    }
        ...
        self.renderer
            .render(Context::render(&mut self.imgui), &gpu_context.queue, &gpu_context.device, &mut rpass)
    ...
    pub fn combobox(&mut self, combo_name: &str, items: Vec<&str>) -> String {
        if let Some(value) = self.combos.get(combo_name) {
            ...
            let selected = &self
                .combos
                .get(combo_name)
                .expect(&format!("Somehow combobox {} did not get added ", combo_name))
                .1[0];
            selected.to_string()
        }
    }
